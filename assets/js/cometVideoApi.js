...
cometVideoApi.getBandwidth = function(opt)
{ 
    return {
        video:cometVideoApi.opt.bandwidth_video,
        audio:cometVideoApi.opt.bandwidth_audio,
...
cometVideoApi.ping = function(pipeName, lonelinessTimeout)
{
    ...
    cometApi.web_pipe_send(pipeName, "ping", { status:"ping"})
    ...
    cometApi.subscription("msg.sys_sipCall", function(callEvent)
    {
                ...
                && cometVideoApi.opt.activeCallEvent.data.sys.callPipe == callPipeName)
        {
            if(!cometVideoApi.opt.activeCallEvent.data.sys.conference) 
            {
                ...
                cometApi.web_pipe_send(callPipeName, "out", { status:"busy", callKey: callEvent.data.sys.callKey})
                return;
            }
            ...
            cometApi.web_pipe_send(callPipeName, "accept", {status:"start_accept"})

            // Ответим что мы тут если кто то сомневается
            cometApi.web_pipe_send(callPipeName, "pong", { status:"pong"})
        ...
        callEvent._callPipeId = callPipeName;
        callEvent.pipe_subscription_id = cometApi.subscription(callPipeName, function(event)
        {
                    ...
                    && cometVideoApi.opt.activeCallEvent.data.sys.callPipe == pipeName)
            {
                ...
                cometApi.web_pipe_send(pipeName, "pong", { status:"pong"})
            }
        ...
        {
            let message = cometVideoApi.opt.lastCallEvent.data.message
            if(typeof message == 'string')
            {
                try{ 
                    ...
                    try{  
                        message = JSON.parse(cometApi.stripslashes(message))
                    }catch (e) {

                    } 
                } 
            }
            cometVideoApi.opt.onCall({data:message});
        }
        else
        {
...
cometVideoApi.busy = function()
{
    ...
    cometApi.web_pipe_send(callPipe, "out", { status:"busy", callKey:cometVideoApi.opt.lastCallEvent.data.sys.callKey})
...
cometVideoApi.callStart = function(pipe, person, status)
{
    ...
    if(person == "interlocutor")
    {
        ...
        if(call.call_media_self_status && status != "answer")
        {
            ...
            cometApi.web_pipe_send(pipe, "mediaStart", { status:"answer"})
        }
    }
    if(person == "self")
    {
        ...
        cometApi.web_pipe_send(pipe, "mediaStart", { status:"self"})
    }
...
cometVideoApi.acceptCall = function(opt)
{
    ...
    cometVideoApi.opt.activeCallEvent._trackPipeId = "track_"+callPipeName;
    cometApi.subscription("track_"+callPipeName,function(event)
    {
    ...
    cometVideoApi._sipUnRegister(function()
    {
        ...
        {
            // Уведомляем что начали взятие трубки
            cometApi.web_pipe_send(callPipeName, "accept", {status:"start_accept", type:opt.type})
        ...
        if(cometVideoApi.opt.activeCallEvent.call_options.custom_stream)
        {
            if(opt.video_local)
            {
                ...
                opt.video_local.srcObject = cometVideoApi.opt.activeCallEvent.call_options.custom_stream
            }
            ...
            else
            {
                ...
                navigator._getUserMedia({audio: true, video: opt.type == 'video'}, function (stream)
                {
                    ...
                    if(opt.video_local)
                    {
                        ...
                        opt.video_local.srcObject = stream
                    }
        ...
        var eventHandlers = {
            ...
            'addstream':  function(event)
            {
                ...
                var intervalId = setInterval(function()
                {
                    ...
                    cometVideoApi.opt.activeCallEvent.call_options.video_remote.srcObject = event.stream;
...
cometVideoApi.muteAudio = function(setMute)
{
    if(cometVideoApi.opt.activeCallEvent && cometVideoApi.opt.activeCallEvent.jssipRTCSession)
    {
        ...
        cometApi.web_pipe_send(cometVideoApi.opt.activeCallEvent.data.sys.callPipe, "toggleMute", { isMute:setMute })
        return true
    }
    return undefined
}
...
cometVideoApi.hangup = function(callInfo, doNotNotify)
{
    ...
    if(call)
    {
        ...
        if(!doNotNotify)
        {
            ...
            cometApi.web_pipe_send(callPipeName, "out", { status:"hangup", callKey:call.data.sys.callKey})
        }
    ...
    else if(cometVideoApi.opt.lastCallEvent && !cometVideoApi.opt.lastCallEvent._isHangup)
    {
        ...
        if(!doNotNotify)
        {
            cometApi.web_pipe_send(callPipeName, "out", { status:"hangup", callKey:cometVideoApi.opt.lastCallEvent.data.sys.callKey})
        }
        if(cometVideoApi.opt.lastCallEvent._callPipeId) cometApi.unsubscription(cometVideoApi.opt.lastCallEvent._callPipeId)
